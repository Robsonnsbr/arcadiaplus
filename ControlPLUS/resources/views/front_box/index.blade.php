            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        @if ($adm)
                            <div class="col-md-2">
                                {!! Form::select('user_id', 'Usuário', ['' => 'Selecione'] + $usuarios->pluck('name', 'id')->all())->attrs([
                                    'class' => 'select2',
                                ]) !!}
                            </div>
                        @endif
                        <div class="col-md-4 text-left">
                            ...
                            <a id="clear-filter" class="btn btn-danger" href="{{ route('frontbox.index') }}"><i
                            ...
                            <div class="col-md-3">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <p class="text-danger">Data de ínicio:
                    ...
                    <div class="col-lg-12 mt-4">
                        {{-- ALTERAÇÃO #2: permitir overflow vertical do dropdown dentro de table-responsive --}}
                        <div class="table-responsive" style="overflow-x:auto; overflow-y:visible; position:relative;">
                                        ...
                                        <tr>
                                            <td class="text-start d-none d-md-table-cell">
                                                <div class="dropdown">
                                                    <button class="btn btn-primary btn-sm dropdown-toggle" type="button"
                                                        data-bs-toggle="dropdown" {{-- ALTERAÇÃO #1: usar boundary viewport --}}
                                                        data-bs-boundary="viewport" style="position: relative; z-index: 1;">
                                                        Ações
                                                    </button>
                                                    ...
                                                    <ul class="dropdown-menu  shadow"
                                                        ...
                                                        <form action="{{ route('frontbox.destroy', $item->id) }}"
                                                            ...
                                                            <li>
                                                                <button type="button" class="dropdown-item"
                                                                    ...
                                                                    <i class="ri-printer-line me-1 text-primary"></i>
                                                                    Imprimir Não Fiscal
                                                                </button>
                                                            </li>
                                                            ...
                                                            @if ($item->estado == 'aprovado')
                                                                <li>
                                                                    <a class="dropdown-item text-info" target="_blank"
                                                                        href="{{ route('nfce.imprimir', [$item->id]) }}">
                                                            ...
                                                            @if ($item->cliente && sizeof($item->fatura) > 0)
                                                                <li>
                                                                    <a class="dropdown-item" target="_blank"
                                                                        href="{{ route('frontbox.imprimir-carne', [$item->id]) }}">
                                                            ...
                                                            @if ($envioWppLink)
                                                                <li>
                                                                    <button type="button"
                                                                        class="dropdown-item text-success"
                                                                        onclick="enviarWpp('{{ $item->id }}', 'nfce')">
                                                                        <i class="ri-whatsapp-fill me-1"></i> Enviar
                                                                        WhatsApp
                                                                    </button>
                                                                </li>
                                                            @endif
                                            ...
                                            <td class="d-md-none">
                                                <form action="{{ route('frontbox.destroy', $item->id) }}" method="post"
                                                    id="form-{{ $item->id }}" style="width: 380px">
                                                    @method('delete')
                                                    @csrf

                                                    <a title="Imprimir não fiscal"
                                                        onclick="imprimir('{{ $item->id }}')"
                                                        class="btn btn-primary btn-sm">
                                                        <i class="ri-printer-line"></i>
                                                    </a>

                                                    <a title="Imprimir A4" onclick="imprimirA4('{{ $item->id }}')"
                                                    ...
                                                    @if ($item->estado == 'novo' || $item->estado == 'rejeitado')
                                                        ...
                                                        @can('pdv_edit')
                                                            <a class="btn btn-warning btn-sm" title="Editar venda"
                                                                href="{{ route('frontbox.edit', $item->id) }}">
                                                    ...
                                                    @if ($item->estado != 'aprovado')
                                                        ...
                                                        <a target="_blank" title="XML temporário"
                                                            class="btn btn-dark btn-sm"
                                                            href="{{ route('nfce.xml-temp', $item->id) }}">
                                                    ...
                                                    @if ($item->estado == 'aprovado')
                                                        <a class="btn btn-info btn-sm" title="Imprimir NFCe"
                                                            target="_blank"
                                                            href="{{ route('nfce.imprimir', [$item->id]) }}">
                                                            <i class="ri-printer-line"></i>
                                                        </a>
                                                    ...
                                                    @if ($item->cliente && sizeof($item->fatura) > 0)
                                                        <a target="_blank" title="Imprimir carnê"
                                                            href="{{ route('frontbox.imprimir-carne', [$item->id]) }}"
                                            ...
                                            <td data-label="Ambiente">
                                                {{ $item->ambiente == 2 ? 'Homologação' : 'Produção' }}</td>

                                            <td data-label="Número NFCe">
                                                <label
                                                    style="width: 100px">{{ $item->estado == 'aprovado' ? $item->numero : '--' }}</label>
                                            </td>
                                            ...
                                            <td data-label="Lista de preço">
                                                <label
                                                    style="width: 100px">{{ $item->lista ? $item->listaPreco->nome : '--' }}</label>
                                            </td>
                    ...
                    <h5 class="mt-2">VALOR TOTAL DAS VENDAS: <strong class="text-success">R$
                            {{ __moeda($somaGeral) }}</strong></h5>
                </div>
            </div>
        </div>
    </div>
...
@section('js')
    ...
    {{-- ALTERAÇÃO #3: fix definitivo do dropdown em tabela (hover/z-index) --}}
    <script type="text/javascript">
